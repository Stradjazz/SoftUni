    ...
    class GeometryCalc
    {
        static void Main(string[] args)
        {
            ...
            if (figure == "triangle")
            {
                ...
                Console.WriteLine($"{Math.Round(AreaOfTriangle(triangleSide, triangleHeight), 2):F2}");
            }
            else if (figure == "square")
            {
                double squareSide = double.Parse(Console.ReadLine());
                Console.WriteLine($"{AreaOfSquare(squareSide):F2}");
            }
            else if (figure == "rectangle")
            {
                double rectangleSide = double.Parse(Console.ReadLine());
                double rectangleHeight = double.Parse(Console.ReadLine());
                Console.WriteLine($"{Math.Round(AreaOfRectangle(rectangleSide, rectangleHeight), 2):F2}");
            }
            else if (figure == "circle")
            {
                double radius = double.Parse(Console.ReadLine());
                Console.WriteLine($"{Math.Round((AreaOfCircle(radius)), 2):F2}");
            }
