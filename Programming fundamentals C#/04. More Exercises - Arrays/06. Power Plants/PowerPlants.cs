...
namespace _06.Power_Plants
{
    /*  Ever heard of Power Plants? Power Plants are a special type of plant which collectively withers and blooms in strange ways.
        ...
        which blooms instead. The special plant which blooms is the one whose index corresponds to the current day. Its
        power is increased by 1, but only if it isn’t dead (but since it has withered the same day, realistically its power stays the same).
